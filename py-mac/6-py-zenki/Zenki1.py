...
class CounterManager(dict):
    """
    A dict like counter manager.

    When using a tuple as event key, say: ('foo', 'bar')

    It's useful for a group of counters.
    """

    def add_counter(self, key, cls=TimebaseAverageWindowCounter):
        self.__setitem__(key, cls)
    ...
    def event(self, key, value=1):
        ...
        assert key in self.keys(), "event key not exists"
        self.__getitem__(key).event(value)
        return self
